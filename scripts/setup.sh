...
/opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P dbatools.IO -d master -i /dbatools-setup/create-admin.sql
...
if [ -f "/dbatools-setup/primary" ]; then
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sqladmin -P dbatools.IO -d master -Q "EXEC sp_addserver 'mssql1', local"
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sqladmin -P dbatools.IO -d master -i /dbatools-setup/restore-db.sql
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sqladmin -P dbatools.IO -d master -i /dbatools-setup/create-objects.sql
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sqladmin -P dbatools.IO -d master -i /dbatools-setup/create-regserver.sql
else
...
/opt/mssql-tools/bin/sqlcmd -S localhost -U sqladmin -P dbatools.IO -d master -i /dbatools-setup/create-endpoint.sql